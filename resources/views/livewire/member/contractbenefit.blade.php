                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="border-b whitespace-nowrap text-right">{{ number_format($row->amount, 2) }}</td>
                            <td class="border-b whitespace-nowrap text-right">
                                @if (!$row->id_withdrawal)
                                <button class="btn btn-xs btn-success" wire:click="claimPassive({{ $row->getKey() }})">
                                    Claim
                                </button>
                                @else
                                @if ($row->withdrawal->processed_at)
                                    Withdrawal success, {{ $row->withdrawal->processed_at }} : {{ $row->withdrawal->txid }}
                                @else
            ...
            <div class="alert alert-dark show intro-x">
                <form wire:submit.prevent="claimContract">
                    <div class="mt-3">
                        ...
                        <input id="contract" type="text" class="form-control text-gray-700" readonly value="{{ number_format($contract) }}" placeholder="contract">
                        @error('contract')
