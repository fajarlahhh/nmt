...
class Dashboard extends Component
{
  ...
  public function contractSubmit()
  {
    if (auth()->user()->invalid_at < now()) {
      ...
      DB::transaction(function () {
        ...
        bonusIncome::where('id_user', auth()->id())->delete();
  ...
  public function bonusMount()
  {
    $this->bonusData = auth()->user()->bonus_income;
  }

  public function bonusSubmit($key)
  {
    $data = bonusIncome::where('id', $key)->whereNull('id_withdrawal')->where('valid_at', '<=', now())->get();
    if ($data->count() > 0) {
      $data = $data->first();
      $error = null;
      ...
      DB::transaction(function () use ($data, $key) {
        ...
        $withdrawal->usdt_amount = $this->usdtWd;
        $withdrawal->id_user = auth()->id();
        $withdrawal->type = 'bonus';
        $withdrawal->save();

        bonusIncome::where('id', $key)->update([
