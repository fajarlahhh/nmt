...
class Enrollment extends Component
{
  ...
  public function render()
  {
    ...
    return view('livewire.member.enrollment')->extends('layouts.default', [
      'menu' => 'enrollment',
      'entrant' => User::select(DB::raw("*, LENGTH(REPLACE(network, '" . $user . "', '')) - LENGTH(REPLACE(REPLACE(network, '" . $user . "', ''), '.', '')) + 1 level"))->with('contract')->where('network', 'like', $user . '%')->whereRaw("LENGTH(REPLACE(network, '" . $user . "', '')) - LENGTH(REPLACE(REPLACE(network, '" . $user . "', ''), '.', '')) < 5")->get(),
    ]);
  }
}
